...
func securityClaims() []claim {
	...
	return []claim{
		...
		{"global_window", d.GlobalWindow.String(),
			"Rolling window both global limbs are measured over. 0 = total mode: nothing ages out, and unlike aggregate_window it is DURABLE, so an exhausted ceiling survives a restart until an operator raises a limb or removes the ledger.",
