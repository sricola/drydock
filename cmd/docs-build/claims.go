...
type claim struct {
	...
	// Tests are the Go tests enforcing this row. A row that states several
	// distinct properties needs several tests, and naming one of them was a
	// quiet way to be wrong: the page promises "the test that enforces it", so
	// a row whose cited test covers only its first sentence claims verification
	// it does not have. Every name here is checked to exist
	// (TestSecurityDefaultsVerifiedByTestsExist), and each must actually
	// enforce some claim the row makes.
	Tests []string
}
...
func securityClaims() []claim {
	...
	return []claim{
		{"task_budget_usd", fmt.Sprintf("%.2f", d.TaskBudgetUSD),
			...
			[]string{"TestGateway_OverBudget402"}},
		{"task_max_inflight", fmt.Sprintf("%d", d.TaskMaxInFlight),
			...
			[]string{"TestAdmit_InFlightLimit"}},
		{"task_max_requests", fmt.Sprintf("%d (0 falls closed to %d)", d.TaskMaxRequests, broker.DefaultUncappedRequestCap),
			...
			[]string{"TestRequestCap_RejectsOverLimit"}},
		{"max_request_cost_usd", fmt.Sprintf("%.2f (0 = reservation off)", d.MaxRequestCostUSD),
			...
			[]string{"TestAdmit_InFlightReservationBounds"}},
		...
		{"global_max_tasks", fmt.Sprintf("%d (0 = off)", d.GlobalMaxTasks),
			"Cumulative TASK STARTS across all vendors and both auth modes over global_window. Hard at admission (the count is claimed in the same critical section as the check, so concurrent admissions cannot overshoot it). This is the limb that bounds subscription mode and the backstop for every USD metering gap, because it counts an event the broker causes rather than dollars a response reported. Must be >= max_concurrent_tasks.",
			[]string{
				"TestGlobalCap_ConcurrentAdmissionsCannotOvershoot",
				"TestGlobalCeiling_SubscriptionModeIsBoundedByTheTaskLimb",
				"TestGlobalCeiling_RetryStormIsBoundedEndToEnd",
				"TestGlobalCeiling_RejectsLimbBelowConcurrency",
			}},
		{"global_window", d.GlobalWindow.String(),
			"Rolling window both global limbs are measured over; strictly after the cutoff, matching the gateway's own window arithmetic. 0 = total mode: nothing ages out, and unlike aggregate_window it is DURABLE, so an exhausted ceiling survives a restart until an operator raises a limb or removes the ledger.",
			[]string{
				"TestGlobalCap_WindowRollsOver",
		...
		{"task_timeout", d.TaskTimeout.String(),
			...
			[]string{"TestHandleTask_TimeoutTerminatesAndDoesNotPush"}},
		{"verify.repos.*.timeout", fmt.Sprintf("%s per command when the repo's config leaves it unset", broker.DefaultVerifyTimeout),
			...
			[]string{"TestRunVerify_TimeoutIsInconclusiveNeverPassed"}},
		{"profiles.repos.*.timeout", fmt.Sprintf("%s per command when the repo's profile leaves it unset", broker.DefaultSetupTimeout),
			...
			[]string{"TestRunSetup_TimeoutForceDeletesVM"}},
		{"stage_quota_gb", fmt.Sprintf("%d", d.StageQuotaGB),
			...
			[]string{"TestQuota_HardBoundENOSPC"}},
		{"(built-in) stage soft bounds", fmt.Sprintf("%d GiB, %d files, %d GiB host free floor",
			...
			[]string{"TestHandleTask_StageFillTerminatesAndDoesNotPush"}},
		...
		{"(built-in) review diff cap", fmt.Sprintf("%d MiB", int64(stage.MaxDiffBytes)>>20),
			...
			[]string{"TestCaptureDiff_OversizeDiffFailsClosed"}},
	}
}
...
func renderClaims(claims []claim) string {
	...
	b.WriteString("What the shipped defaults bound, how hard each bound is, and the tests that enforce it. ")
	...
	for _, c := range claims {
		...
		for _, n := range c.Tests {
			names = append(names, "`"+n+"`")
		}
		fmt.Fprintf(&b, "| `%s` | %s | %s | %s |\n", c.Setting, c.Default, c.Bounds, strings.Join(names, "<br>"))
	}
