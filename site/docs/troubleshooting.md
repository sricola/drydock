...
| A `config.yaml` edit or env var isn't taking effect | `drydock policy explain` — the SOURCE column shows which layer (default / config.yaml / env:VAR) actually won for each setting, and the daemon verdict at the bottom catches the other classic cause: brokerd resolves its policy once at boot, so a `DIVERGENT` verdict means the running daemon predates your change — restart brokerd to pick it up. See [Configuration](configuration.html#inspecting-the-effective-policy). |
