...
the `submit`/`doctor` CLI.

**Host topology caveat.** The push and `gh pr create` run on the **brokerd**
host. In the default setup `drydock submit` talks to brokerd over a unix socket
on the same machine, so a submit-side CLI check is representative. But when
`BROKER_ADDR` is set (`client.go:54`), submit talks to a brokerd over TCP and
may run on a *different* host — where the local `gh` says nothing about the
brokerd host. So the submit-side preflight (component 4) is a **same-host
heuristic only**, skipped when `BROKER_ADDR` is set. The topology-independent
guarantees are the broker-side **graceful degrade** (component 2) and `doctor`
run on the brokerd host.

**Core safety property.** Because the broker treats `OpenRequest` as best-effort
(component 2), the *entire class* of PR-step failures — CLI missing, not
authenticated, a wrong vendor flag, an already-open PR, a transient network
error — degrades to a clear `pushed; PR not opened — open it manually`, never a
`push failed`. The branch is always saved. This is the design's central win and
what de-risks the per-vendor flag details.
...
`prContent(instruction, taskID)` (broker helper): `title =
truncate(firstLine(instruction), 72)` (PR titles must stay short — a long first
line is clipped, with an `…` suffix when clipped); `body = instruction +
"\n\n---\nGenerated by drydock (task " + taskID + ")."`. Empty instruction →
`("", "")`, so adapters fall back to `--fill`.

**Bounded body, argv-safe.** A task instruction can be multi-KB, so passing the
whole thing as `--body <argv>` risks `E2BIG`/`ARG_MAX` on the exec. Rather than
depend on per-CLI `--body-file` support (which gh has but glab/tea may not),
`prContent` **caps the body** to a safe bound (the first ~4 KB of the
instruction; if clipped, append `\n\n[truncated — full instruction in the
drydock task audit]`). This keeps argv bounded uniformly across all three CLIs
with no per-vendor branching, and the full instruction is already preserved in
`<audit>/<id>.jsonl`. The title is likewise bounded (72 chars).
...
- **`drydock submit`** (client-side, repo-aware): after building the request,
  **and only when `BROKER_ADDR` is unset** (otherwise brokerd may be on another
  host and the local `gh` is irrelevant — see the topology caveat), build
  `adapter := remote.AdapterFor(repo, platform)`; if `err := adapter.Available();
  err != nil`, print to stderr (non-blocking):
  `⚠ <name> CLI unavailable on this host (<reason>): the task will run and push a
  branch, but the PR won't open automatically. Fix it (e.g. 'gh auth login') and
  open the PR manually, or pass --platform none.` Then proceed with the submit.
  This is a same-host convenience warning, not a guarantee; the broker-side
  graceful degrade is the real net.
...
- A stage whose `Push` fails yields the `push failed` error event (fatal path
  preserved).
- `prContent`: non-empty instruction → title=first line (≤72 chars, `…` when
  clipped), body contains the instruction + provenance footer; empty instruction
  → ("",""). A multi-KB instruction → body capped at ~4 KB with the truncation
  note; title still ≤72 chars (guards the argv-size fix).
- submit skips the `Available()` warning when `BROKER_ADDR` is set, and emits it
  (on `Available()` error) when unset.
