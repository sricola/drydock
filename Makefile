...
help:
	...
	@echo "  redteam-vm  run the VM-backed attacks (A1/A2/A7/A8/V1); macOS + container runtime"
	@echo "  release-preflight  full pre-release gate: unit + host (A3-A6) + VM (A1/A2/A7/A8/V1)"
...
# here and in CI. VM-backed claims (A1, A2, A7, A8, V1) need the sandbox — run
...
redteam:
	...
	@echo "== host-side containment verified. VM-backed A1, A2, A7, A8, V1: make redteam-vm =="
...
release-preflight: build image network
	...
	go test -count=1 -run '$(REDTEAM)' ./...
	@echo "== release preflight [3/3]: VM-backed red-team (A1, A2, A7, A8, V1) =="
	go test -tags=integration -count=1 -timeout=10m -run 'TestRedteam_' ./tests/...
	@echo ""
	@echo "== release preflight GREEN: unit + host (A3-A6) + VM (A1/A2/A7/A8/V1) all pass =="
